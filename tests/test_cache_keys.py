...
def test_deterministic_pagination():
    ...
    test_data = [
        {"group_id": "g1", "group_size": 3, "max_score": 0.9, "primary_name": "Company A", "disposition": "keep"},
        {"group_id": "g2", "group_size": 2, "max_score": 0.8, "primary_name": "Company B", "disposition": "keep"},
        {"group_id": "g3", "group_size": 4, "max_score": 0.7, "primary_name": "Company C", "disposition": "merge"},
        {"group_id": "g4", "group_size": 1, "max_score": 0.6, "primary_name": "Company D", "disposition": "merge"},
        {"group_id": "g5", "group_size": 2, "max_score": 0.5, "primary_name": "Company E", "disposition": "keep"},
    ]
    ...
    try:
        ...
        with unittest.mock.patch('src.utils.group_pagination.get_artifact_paths') as mock_paths:
            ...
            with unittest.mock.patch('src.utils.settings.get_settings') as mock_settings:
                ...
                # Get page 1
                page1_data, total1 = get_groups_page("test_run", "Group Size (Asc)", 1, page_size, {})
                # Get page 2  
                page2_data, total2 = get_groups_page("test_run", "Group Size (Asc)", 2, page_size, {})
                ...
                assert page1_last_size <= page2_first_size, f"Page ordering broken: {page1_last_size} should be <= {page2_first_size}"
                
                # Verify stable ordering when re-fetching same page
                page1_data_again, _ = get_groups_page("test_run", "Group Size (Asc)", 1, page_size, {})
...
def test_stats_path_no_aliasing():
...
def test_stats_path_with_filters():
    ...
    test_data = [
        {'group_id': 'g1', 'group_size': 3, 'max_score': 0.9, 'primary_name': 'Company A', 'disposition': 'keep'},
        {'group_id': 'g2', 'group_size': 2, 'max_score': 0.8, 'primary_name': 'Company B', 'disposition': 'merge'},
        {'group_id': 'g3', 'group_size': 1, 'max_score': 0.3, 'primary_name': 'Company C', 'disposition': 'delete'},
        {'group_id': 'g4', 'group_size': 4, 'max_score': 0.7, 'primary_name': 'Company D', 'disposition': 'keep'},
    ]
    df = pd.DataFrame(test_data)
    ...
    try:
        ...
        with unittest.mock.patch('src.utils.group_pagination.get_artifact_paths') as mock_paths, \
             unittest.mock.patch('src.utils.group_pagination.DUCKDB') as mock_duckdb:
            ...
            mock_conn.execute.side_effect = [mock_pragma_result, mock_result, mock_count_result]
            
            # Test stats path with filters
            page, total = get_groups_page_from_stats_duckdb(
                'test_run', "Max Score (Desc)", page=1, page_size=10,
            ...
            assert mock_conn.execute.call_count == 3  # PRAGMA + main query + count query
            ...
            assert "WHERE disposition IN (?,?) AND max_score >= ?" in second_call_args[0][0]  # SQL contains filters
            assert len(second_call_args[0][1]) == 6  # group_stats_path + 2 dispositions + min_edge_strength + page_size + offset
            ...
            assert "WHERE disposition IN (?,?) AND max_score >= ?" in third_call_args[0][0]  # SQL contains filters
            assert len(third_call_args[0][1]) == 4  # group_stats_path + 2 dispositions + min_edge_strength
...
def test_pyarrow_details_group_id_filtering():
    ...
    test_data = [
        ...
        {'group_id': 'g1', 'account_name': 'Company A2', 'is_primary': False, 'weakest_edge_to_primary': 0.8, 'disposition': 'keep'},
        {'group_id': 'g2', 'account_name': 'Company B', 'is_primary': True, 'weakest_edge_to_primary': 0.7, 'disposition': 'merge'},
        {'group_id': 'g3', 'account_name': 'Company C', 'is_primary': True, 'weakest_edge_to_primary': 0.6, 'disposition': 'delete'},
    ]
    df = pd.DataFrame(test_data)
    ...
    try:
        ...
        assert all(row['group_id'] == 'g1' for row in result), f"Expected all rows to have group_id='g1', got: {[row['group_id'] for row in result]}"
        assert len(result) == 2
        ...
        result_filtered, total_filtered = _get_group_details_pyarrow(
            parquet_path, 'g1', 'account_name ASC', 1, 10, 
            {'dispositions': ['keep']}, mock_settings
        )
...
def test_config_validation_edge_cases():
    ...
    assert any("ui.max_pyarrow_groups_seconds must be number 1-60" in w for w in warnings)


def test_force_backend_flags():
    ...
    with unittest.mock.patch.dict(os.environ, {"CJ_FORCE_PYARROW": "1"}):
        ...
        try:
            ...
            result = get_groups_page("test_run", "Group Size (Desc)", 1, 10, {})
...
def test_force_backend_flags_precedence():
    ...
    with unittest.mock.patch.dict(os.environ, {"CJ_FORCE_PYARROW": "1", "CJ_FORCE_DUCKDB": "1"}):
        try:
...
def test_force_backend_flags_precedence_details():
    ...
    with unittest.mock.patch.dict(os.environ, {"CJ_FORCE_PYARROW": "1", "CJ_FORCE_DUCKDB": "1"}):
        try:
            # This should force DuckDB backend due to precedence rule
            result = get_group_details("test_run", "test_group", "account_name ASC", 1, 10, {})
            # If we get here, DuckDB was forced (or we hit an expected error)
            print("✅ Force flag precedence working in group_details (DuckDB wins)")
        except Exception as e:
            # Expected if test data doesn't exist
            print(f"✅ Force flag precedence working in group_details (expected error: {e})")


def test_pyarrow_projection_safety():
    ...
    try:
        ...
        mock_settings = {"ui": {"max_page_size": 250}}
        
        # This should handle missing columns gracefully
        result = get_groups_page_pyarrow("test_run", "account_name ASC", 1, 10, {})
...
def test_stats_path_threads_cap():
    ...
    with unittest.mock.patch('src.utils.settings.get_settings', return_value=mock_settings):
        try:
            # This should cap threads to 32 in the PRAGMA
            result = get_groups_page_from_stats_duckdb("test_run", "account_name ASC", 1, 10, {})
