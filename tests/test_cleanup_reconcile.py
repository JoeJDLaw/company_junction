...
class TestCleanupReconcile:
    ...
    def test_discover_candidates_without_reconcile(self):
        ...
        run_index = {
            "run_123": {"timestamp": "2024-01-01T00:00:00Z", "input_paths": ["test.csv"]},
            "run_456": {"timestamp": "2024-01-02T00:00:00Z", "input_paths": ["test2.csv"]},
        }
    ...
    def test_discover_candidates_with_reconcile(self):
        ...
        run_index = {
            "run_123": {"timestamp": "2024-01-01T00:00:00Z", "input_paths": ["test.csv"]},
            "stale_run": {"timestamp": "2024-01-02T00:00:00Z", "input_paths": ["test2.csv"]},
        }
        
        # Mock filesystem scanning to return orphan and stale runs
        with pytest.MonkeyPatch().context() as m:
            m.setattr("tools.cleanup_test_artifacts.scan_filesystem_runs", lambda: {"run_123", "orphan_run"})
    ...
    def test_stale_index_detected(self, tmp_path):
        ...
        index_data = {
            "stale_abc": {"timestamp": "2024-01-01T00:00:00Z", "input_paths": ["test.csv"]}
        }
    ...
    def test_dry_run_flag_explicit(self):
        ...
        # Test discover_candidates without reconciliation (existing behavior)
        run_index = {
            "test_run": {"timestamp": "2024-01-01T00:00:00Z", "input_paths": ["test.csv"]}
        }
...
class TestCleanupReconcileIntegration:
    ...
    def test_full_reconciliation_workflow(self):
        ...
        run_index = {
            "valid_run": {"timestamp": "2024-01-01T00:00:00Z", "input_paths": ["valid.csv"]},
            "stale_run": {"timestamp": "2024-01-02T00:00:00Z", "input_paths": ["stale.csv"]},
        }
