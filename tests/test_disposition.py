...
from disposition import (
    classify_disposition, _is_blacklisted, _is_suspicious_singleton,
    apply_dispositions, compute_group_metadata
)
...
class TestDisposition(unittest.TestCase):
    ...
    def test_blacklist_detection(self):
        ...
        for name in good_names:
            self.assertFalse(_is_blacklisted(name), f"'{name}' should not be blacklisted")
    ...
    def test_apply_dispositions(self):
        ...
        self.assertEqual(df_dispositions.iloc[2]['Disposition'], 'Delete')  # 'PNC is not sure'
        self.assertEqual(df_dispositions.iloc[3]['Disposition'], 'Delete')  # '1099, no paystubs'
    ...
    def test_multiple_names_verification(self):
        ...
        self.assertEqual(df_dispositions.iloc[0]['disposition_reason'], 'multi_name_string_requires_split')
    ...
    def test_manual_override_application(self):
        ...
        try:
            ...
            self.assertEqual(result.iloc[0]['disposition_reason'], 'manual_override:Delete')
    ...
    def test_disposition_reasons(self):
        ...
        self.assertEqual(df_dispositions.iloc[2]['disposition_reason'], 'blacklisted_name')  # 'PNC is not sure'
        self.assertEqual(df_dispositions.iloc[0]['disposition_reason'], 'clean_singleton')  # Normal record
        
        # Normal names should be Keep (singletons)
        self.assertEqual(df_dispositions.iloc[0]['Disposition'], 'Keep')  # '20-20 Plumbing & Heating Inc'
        self.assertEqual(df_dispositions.iloc[4]['Disposition'], 'Keep')  # 'Acme Corporation'
