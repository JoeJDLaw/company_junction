...
@pytest.fixture
def synthetic_10k_data() -> pd.DataFrame:
...
@pytest.fixture
def synthetic_100k_data() -> pd.DataFrame:
...
@pytest.fixture
def synthetic_1m_data() -> pd.DataFrame:
...
def synthetic_10k_paths(
    ...
    stats_data = (
        synthetic_10k_data.groupby("group_id")
        .agg(
            {
                "group_size": "count",
                "max_score": "max",
                "primary_name": lambda x: x.iloc[0],  # First primary name
                "disposition": lambda x: (
                    x.mode().iloc[0] if len(x.mode()) > 0 else "keep"
                ),
            },
        )
        .reset_index()
...
def synthetic_100k_paths(
    ...
    # Create group_stats parquet (aggregated)
    stats_data = (
        synthetic_100k_data.groupby("group_id")
        .agg(
            {
                "group_size": "count",
                "max_score": "max",
...
def synthetic_1m_paths(
    ...
    # Create group_stats parquet (aggregated)
    stats_data = (
        synthetic_1m_data.groupby("group_id")
        .agg(
            {
                "group_size": "count",
                "max_score": "max",
...
class TestFilterVariants:
    ...
    def test_filter_variants_10k(self, benchmark, synthetic_10k_paths, monkeypatch):
        ...
        for filter_dict in filter_combinations:
            filters: Dict[str, Any] = filter_dict
