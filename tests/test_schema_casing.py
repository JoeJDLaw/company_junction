...
class TestSchemaCasing:
    ...
    def test_schema_casing_disposition_lowercase(self):
        ...
        try:
            ...
            assert (
                "disposition" in df.columns
            ), "Expected lowercase 'disposition' column"
            assert (
                "Disposition" not in df.columns
            ), "Unexpected uppercase 'Disposition' column"
    ...
    def test_schema_casing_review_ready_lowercase(self):
        ...
        try:
            ...
            assert (
                "disposition" in df.columns
            ), "Expected lowercase 'disposition' column in review_ready.parquet"
            assert (
                "Disposition" not in df.columns
            ), "Unexpected uppercase 'Disposition' column in review_ready.parquet"
    ...
    def test_schema_casing_backend_specific_files(self):
        ...
        try:
            ...
            if Path(duckdb_path).exists():
                ...
                assert (
                    "disposition" in df_duckdb.columns
                ), "DuckDB file missing lowercase 'disposition'"
            ...
            if Path(pandas_path).exists():
                ...
                assert (
                    "disposition" in df_pandas.columns
                ), "Pandas file missing lowercase 'disposition'"
