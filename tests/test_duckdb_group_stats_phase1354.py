...
def create_test_group_data():
    ...
    return pd.DataFrame({
        ...
        "disposition": ["Keep", "Update", "Keep", "Update", "Update", "Keep", "Delete"]
    })


def test_duckdb_group_stats_engine_creation():
...
def test_duckdb_group_stats_computation():
...
def test_duckdb_memoization():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            print(f"DEBUG: direct_cache_key_test | key1={cache_key1} | key2={cache_key2} | equal={cache_key1 == cache_key2}")
            ...
            print(f"Performance: cache_miss={metadata1['elapsed_sec']:.3f}s, cache_hit={metadata2['elapsed_sec']:.3f}s")
            
        finally:
            engine.close()


def test_duckdb_parquet_write():
...
def test_parquet_size_reporter():
...
def test_performance_improvement():
    ...
    df_large = pd.DataFrame({
        ...
        "weakest_edge_to_primary": [np.random.uniform(80, 100) for _ in range(n_records)],
        "disposition": ["Keep" if is_primary else "Update" for is_primary in is_primary_list]
    })
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            print(f"Performance improvement: {improvement:.1f}% (pandas: {pandas_time:.3f}s, duckdb: {duckdb_time:.3f}s)")
...
def _compute_group_stats_pandas(df_primary: pd.DataFrame) -> pd.DataFrame:
    ...
    from src.utils.schema_utils import GROUP_ID, GROUP_SIZE, MAX_SCORE, PRIMARY_NAME, DISPOSITION, ACCOUNT_NAME, IS_PRIMARY, WEAKEST_EDGE_TO_PRIMARY
    ...
    df_group_stats = df_group_stats.sort_values(GROUP_ID, kind="mergesort").reset_index(drop=True)
