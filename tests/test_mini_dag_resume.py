...
class TestMiniDAGResume:
    ...
    def test_validate_intermediate_files_complete_pipeline(self, mini_dag, interim_dir):
        ...
        for stage in PIPELINE_STAGES:
            assert mini_dag.validate_intermediate_files(stage, interim_dir), f"Stage {stage} failed validation"
    ...
    def test_get_smart_resume_stage_with_validation_failure(self, mini_dag, interim_dir):
    ...
    def test_feature_flag_resume_state_repair(self, mini_dag, interim_dir):
        ...
        with patch('src.utils.mini_dag.RESUME_STATE_REPAIR_ENABLED', False):
            ...
            mini_dag.register("normalization")
            mini_dag.register("filtering", deps=["normalization"])
            mini_dag.complete("filtering")  # Complete filtering without completing normalization
    ...
    def test_logging_enhancements(self, mini_dag, interim_dir, caplog):
        ...
        assert any("Auto-resume decision:" in msg for msg in log_messages), f"Expected 'Auto-resume decision:' in logs: {log_messages}"
        assert any("NEXT_STAGE_READY" in msg for msg in log_messages), f"Expected 'NEXT_STAGE_READY' in logs: {log_messages}"
...
class TestMiniDAGResumeIntegration:
    ...
    def test_full_pipeline_resume_simulation(self, temp_dir):
        ...
        assert resume_stage == "grouping", f"Expected to resume from 'grouping', got '{resume_stage}'"
