...
import pytest
from src.utils.filtering import get_order_by, build_sort_expression
...
class TestSortContextConsistency:
    ...
    def test_order_by_and_sort_expression_consistency(self, sort_key, context, expected_column_family):
        ...
        assert expected_column_family in order_by_column.lower(), (
    ...
    def test_all_sort_keys_have_consistent_mappings(self):
        ...
        sort_keys = [
            "Group Size (Desc)", "Group Size (Asc)",
            "Max Score (Desc)", "Max Score (Asc)", 
            "Account Name (Asc)", "Account Name (Desc)"
        ]
    ...
    def test_sort_direction_consistency(self):
        ...
        for sort_key, expected_order_by_direction, expected_sort_expr_direction in test_cases:
            ...
            assert order_by_direction == expected_order_by_direction, (
                f"ORDER BY direction mismatch for {sort_key}: expected {expected_order_by_direction}, got {order_by_direction}"
            )
            
            assert sort_expr_direction == expected_sort_expr_direction, (
    ...
    def test_tiebreaker_consistency(self):
        ...
        for sort_key in sort_keys:
            for context in ["default", "group_details"]:
                ...
                assert len(sort_expr) >= 2, f"Missing tiebreaker for {sort_key} in {context}"
                ...
                assert tiebreaker_direction == "ascending"  # Tiebreaker should always be ascending
