...
from src.utils.path_utils import get_processed_dir, get_artifact_path


def test_group_artifacts_scoped(tmp_path, monkeypatch):
...
def test_artifact_paths_never_global(tmp_path, monkeypatch):
    ...
    for artifact in artifacts:
        ...
        assert not str(artifact_path).startswith("data/processed/") or str(artifact_path).startswith(f"data/processed/{run_id}/")
        assert not str(artifact_path).startswith("data/interim/") or str(artifact_path).startswith(f"data/interim/{run_id}/")
        
        # Should always be under the specific run_id
        assert str(artifact_path).startswith(f"data/processed/{run_id}/") or \
               str(artifact_path).startswith(f"data/interim/{run_id}/")


def test_run_id_required_for_processed_paths():
...
def test_path_utils_consistency():
