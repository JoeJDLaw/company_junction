...
def test_similarity_outputs_expected_columns():
    df = pd.DataFrame([
    ...
    out = pair_scores(df, settings, enable_progress=False, parallel_executor=None, interim_dir=None)
    # OK if empty (thresholds/logic may filter), but if not empty, columns must exist
    if not out.empty:
        for col in ["id_a", "id_b", "score", "ratio_name", "ratio_set", "jaccard", "num_style_match", "suffix_match", "base_score"]:
