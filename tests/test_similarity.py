...
from similarity import (
    pair_scores, _compute_pair_score, _check_numeric_style_match,
    save_candidate_pairs, load_candidate_pairs
)
...
class TestSimilarity(unittest.TestCase):
    ...
    def test_pair_score_computation(self):
        ...
        score_data = _compute_pair_score(row_a, row_b, self.settings['similarity']['penalty'])
    ...
    def test_candidate_pair_generation(self):
        ...
        if not pairs_df.empty:
            self.assertTrue(all(pairs_df['score'] >= self.settings['similarity']['medium']))
    ...
    def test_inc_vs_inc_high_score(self):
        ...
        if not pairs_df.empty:
            self.assertTrue(all(pairs_df['score'] >= self.settings['similarity']['high']))
