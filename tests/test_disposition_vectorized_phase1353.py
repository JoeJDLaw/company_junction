...
def create_test_disposition_data() -> pd.DataFrame:
...
def test_blacklist_loading_from_config() -> None:
...
def test_vectorized_vs_legacy_identical_output() -> None:
    ...
    for idx in result_vectorized.index:
        ...
        assert (
            vec_simple == leg_simple
        ), f"Reasons not equivalent at {idx}: {vec_reason} vs {leg_reason}"


def test_disposition_classification_correctness() -> None:
...
def test_manual_override_handling():
    ...
    _df = create_test_disposition_data()
    ...
    _manual_overrides = {"A1": "Verify", "A3": "Keep"}
...
def test_performance_improvement():
    ...
    assert (
        vectorized_time < legacy_time
    ), f"Vectorized ({vectorized_time:.3f}s) should be faster than legacy ({legacy_time:.3f}s)"

    # Log performance improvement
    improvement = (legacy_time - vectorized_time) / legacy_time * 100
    print(
        f"Performance improvement: {improvement:.1f}% ({legacy_time:.3f}s -> {vectorized_time:.3f}s)",
    )
