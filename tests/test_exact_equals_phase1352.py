...
def test_build_raw_exact_key():
    ...
    assert build_raw_exact_key(None, settings) == ""
    assert build_raw_exact_key("   ", settings) == ""


def test_find_exact_equals_groups():
    ...
    walmart_inc_group = exact_groups[exact_groups["raw_exact_key"] == "Walmart Inc"].iloc[0]
    assert walmart_inc_group["representative_id"] == "A3"  # min account_id


def test_create_unique_normalized():
...
def test_min_group_size_filtering():
    ...
    df = pd.DataFrame({
        ...
        "Account Name": [
            "Walmart", "Walmart",  # Size 2 (should be filtered out)
            "Target", "Target", "Target",  # Size 3 (should be kept)
            "Costco"  # Singleton
        ]
    })
