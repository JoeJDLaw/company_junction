...
REQUIRED_COLUMNS = {
    ...
    "group_details_parquet": {
        "group_id", "account_id", "account_name", "suffix_class", 
        "created_date", "disposition"
    }
}
...
OPTIONAL_COLUMNS = {
    ...
    "group_details_parquet": {
        "is_primary", "weakest_edge_to_primary", "domain", "email", "phone", "address", "website",
        "industry", "revenue", "employee_count", "founded_year"
    }
}
...
class TestHardcodedColumnAssumptions:
            ...
            try:
                ...
                for pattern in problematic_patterns:
                    ...
                    if re.search(pattern, content, re.IGNORECASE):
                        found_issues.append(f"Hardcoded column assumption in {py_file}: {pattern}")
    ...
    def test_detect_schema_constants_usage(self):
        ...
        for schema_file in schema_files:
            try:
                ...
                if "[" in content and "GROUP_ID" in content and "ACCOUNT_NAME" in content:
                    # This is a potential hardcoded list - check if it's in a constant definition
                    lines = content.split('\n')
                    for i, line in enumerate(lines):
                        if "[" in line and "GROUP_ID" in line and "ACCOUNT_NAME" in line:
                            # Check if this line defines a constant
                            if "=" in line and ("DETAILS_COLUMNS" in line or "GROUP_STATS_COLUMNS" in line):
    ...
    def test_schema_constants_match_contracts(self):
        ...
        try:
            ...
            from src.utils.schema_utils import GROUP_ID, ACCOUNT_ID, ACCOUNT_NAME, SUFFIX_CLASS, CREATED_DATE, DISPOSITION
            
            # Check that DETAILS_COLUMNS matches the expected schema
            expected_details_columns = {GROUP_ID, ACCOUNT_ID, ACCOUNT_NAME, SUFFIX_CLASS, CREATED_DATE, DISPOSITION}
...
class TestLegacyColumnHandling:
    ...
    def test_duckdb_handles_missing_legacy_columns(self, sample_data_without_legacy_columns):
        ...
        with tempfile.NamedTemporaryFile(suffix='.parquet', delete=False) as tmp_file:
            ...
            try:
                ...
                # Test basic query
                result = conn.execute(f"SELECT * FROM read_parquet('{tmp_file.name}')").fetchall()
                assert len(result) == 3
    ...
    def test_pyarrow_handles_missing_legacy_columns(self, sample_data_without_legacy_columns):
        ...
        with tempfile.NamedTemporaryFile(suffix='.parquet', delete=False) as tmp_file:
            ...
            try:
                ...
                filtered = table.filter(pa.compute.equal(table['group_id'], pa.scalar('group1')))
                assert filtered.num_rows == 1
                
            finally:
                os.unlink(tmp_file.name)
    
    def test_duckdb_handles_present_legacy_columns(self, sample_data_with_legacy_columns):
    ...
    def test_pyarrow_handles_present_legacy_columns(self, sample_data_with_legacy_columns):
        ...
        with tempfile.NamedTemporaryFile(suffix='.parquet', delete=False) as tmp_file:
            ...
            try:
                ...
                filtered = table.filter(
                    pa.compute.greater_equal(table['weakest_edge_to_primary'], pa.scalar(0.8))
                )
    ...
    def test_conditional_filtering_skips_missing_columns(self, sample_data_without_legacy_columns):
    ...
    def test_conditional_filtering_works_with_present_columns(self, sample_data_with_legacy_columns):
