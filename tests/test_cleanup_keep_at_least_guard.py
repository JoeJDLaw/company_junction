...
from tools.cleanup_test_artifacts import discover_candidates, CleanupPlan
...
def test_cleanup_keep_at_least_guard_logic():
    ...
    assert len(plan.candidates) == 2, f"Expected 2 candidates, got {len(plan.candidates)}"
    ...
    assert not (not allow_empty_override and keep_min > 0 and remaining_with_override < keep_min)
    ...
    assert not (not allow_empty and keep_min_override > 0 and remaining_with_keep_override < keep_min_override)
...
def test_cleanup_keep_at_least_default_behavior():
    ...
    # Discover candidates
    plan = discover_candidates(run_index, types=['test'])
    assert len(plan.candidates) == 1, f"Expected 1 candidate, got {len(plan.candidates)}"
...
def test_cleanup_keep_at_least_override_behavior():
    ...
    # Discover candidates
    plan = discover_candidates(run_index, types=['test'])
    assert len(plan.candidates) == 3, f"Expected 3 candidates, got {len(plan.candidates)}"
