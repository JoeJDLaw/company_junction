...
def compare_alias_matches(
    ...
    if only_in_legacy:
        legacy_scores = legacy_df[
            legacy_df.apply(get_pair_id, axis=1).isin(only_in_legacy)
        ]
    ...
    if only_in_optimized:
        optimized_scores = optimized_df[
            optimized_df.apply(get_pair_id, axis=1).isin(only_in_optimized)
        ]
