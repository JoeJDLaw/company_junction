...
from utils.parity_validator import assert_parity_or_exit
from utils.parquet_size_reporter import load_or_build_report
...
class RunModesBenchmark:
    ...
    def run_parity_mode(self) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            "--dataset", self.dataset,
            "--mode", "parity",
            "--persist-group-stats-artifacts", "true"
        ]
    ...
    def run_benchmark_sanity(self) -> Dict[str, Any]:
        ...
        for run_num in range(1, 4):
            ...
            if result.returncode == 0:
                ...
                cache_hit = "cache_hit" in result.stdout.lower() or "memoization" in result.stdout.lower()
    ...
    def _clean_run_artifacts(self, mode: str):
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        print(f"🚀 Starting Phase 1.35.7 Run-Modes + Benchmark Sanity Matrix")
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        passed_tests = sum(1 for mode in test_modes if self.results[mode].get("success", False))
        
        # Check specific requirements
        parity_valid = self.results.get("parity", {}).get("parity_validation", {}).get("meets_requirement", False)
        benchmark_success = self.results.get("benchmark_sanity", {}).get("success", False)
        
        # Calculate overall success (all tests passed + specific requirements met)
        overall_success = (passed_tests == total_tests and 
                          parity_valid and 
                          benchmark_success)
    ...
    def _save_results(self):
        ...
        output_path = self.benchmark_dir / f"run_modes_benchmark_{self.dataset}_{timestamp}.json"
    ...
    def print_summary(self):
        ...
        print(f"Parity Validation: {'✅ PASS' if summary.get('parity_validation_passed', False) else '❌ FAIL'}")
        print(f"Benchmark Sanity: {'✅ PASS' if summary.get('benchmark_sanity_passed', False) else '❌ FAIL'}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Phase 1.35.7 Run-Modes + Benchmark Sanity Matrix")
