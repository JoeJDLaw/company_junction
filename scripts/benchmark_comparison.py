...
def create_settings_override(
    ...
    return cast("dict[str, Any]", override_settings)
...
def generate_parity_report(run_id: str, dataset_size: str) -> str:
    ...
    try:
        ...
        if not os.path.exists(duckdb_path) or not os.path.exists(pandas_path):
            logger.warning("Parquet files not found for parity validation")
            return "parity_validation_skipped"
        ...
        # Save parity report
        parity_report_path = (
            f"data/processed/{run_id}_duckdb/parity_report_group_stats.json"
        )
        ...
        return str(parity_report_path)  # Return the file path as string

    except Exception as e:
        logger.warning(f"Failed to generate parity report: {e}")
        return "parity_validation_failed"
...
def main() -> int:
    ...
    parser.add_argument(
        "--mode",
        choices=["alias", "group_stats", "parity"],
        default="alias",
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of runs for group stats (default: 3)",
    )
    parser.add_argument(
    ...
    if args.mode == "alias":
        ...
        logger.info(
            f"Running group stats benchmark on {args.dataset} dataset ({args.runs} runs)...",
        )
        ...
        logger.info(
            f"Running group stats parity validation on {args.dataset} dataset...",
        )
