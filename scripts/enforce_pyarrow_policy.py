...
def find_pyarrow_imports(file_path: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if is_hard_banned:
                if re.search(r'\bimport\s+pyarrow\b|\bfrom\s+pyarrow\b', line, re.IGNORECASE):
                    violations.append({
                        'file': file_path,
                        'line': line_num,
            ...
            # Check for restricted imports in allowed files
                ...
                if re.search(r'\bimport\s+pyarrow\b|\bfrom\s+pyarrow\b', line, re.IGNORECASE):
                    ...
                    is_allowed_import = any(re.search(pattern, line) for pattern in ALLOWED_IMPORT_LINES)
