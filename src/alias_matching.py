...
def compute_alias_matches(df_norm: pd.DataFrame, df_groups: pd.DataFrame, settings: Dict) -> Tuple[pd.DataFrame, Dict]:
    """
    ...
    for idx, record in df_norm.iterrows():
        ...
        for i, (alias, source) in enumerate(zip(normalized_aliases, alias_sources)):
            alias_matches.extend(_score_alias_against_records(
                record, alias, source, df_norm, df_groups, high_threshold
            ))
    
    # Limit results if too many
    if len(alias_matches) > max_alias_pairs:
        logger.warning(f"Limiting alias matches to {max_alias_pairs} (found {len(alias_matches)})")
        capped_blocks = 1
    ...
    if alias_matches:
        ...
        df_matches = pd.DataFrame(columns=[
            'record_id', 'alias_text', 'alias_source', 'match_record_id', 
            'match_group_id', 'score', 'suffix_match'
        ])
...
def _score_alias_against_records(
    ...
    for idx, other_record in df_norm.iterrows():
        ...
        if alias_first_token and other_first_token and alias_first_token != other_first_token:
            continue
        ...
        if score >= high_threshold and suffix_match:
            match_group_id = df_groups.loc[idx, 'group_id'] if idx in df_groups.index else -1
...
def create_alias_cross_refs(df_norm: pd.DataFrame, df_alias_matches: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    df_result['alias_cross_refs'] = [
        cross_refs.get(idx, []) for idx in df_result.index
    ]
