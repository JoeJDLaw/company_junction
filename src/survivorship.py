...
def select_primary_records(df_groups: pd.DataFrame, relationship_ranks: Dict[str, int], settings: Dict) -> pd.DataFrame:
    """
    ...
    for group_id in result_df['group_id'].unique():
        ...
        primary_idx = _select_primary_from_group(group_data, relationship_ranks, settings)
...
def _select_primary_from_group(group_data: pd.DataFrame, relationship_ranks: Dict[str, int], settings: Dict) -> int:
    """
    ...
    for _, record in group_data.iterrows():
        ...
        rank = relationship_ranks.get(relationship, 60)  # Default rank for unknown relationships
    ...
    tie_breakers = settings.get('survivorship', {}).get('tie_breakers', ['created_date', 'account_id'])
    ...
    for tie_breaker in tie_breakers:
        if tie_breaker == 'created_date' and 'Created Date' in group_data.columns:
            ...
            group_data['created_date_sort'] = group_data['Created Date'].apply(excel_serial_to_datetime)
...
def generate_merge_preview(df_groups: pd.DataFrame, selected_fields: Optional[List[str]] = None) -> pd.DataFrame:
    """
    ...
    if selected_fields is None:
        ...
        selected_fields = [
            'Account Name', 'Account ID', 'Relationship', 'Created Date',
            'Account Owner: Full Name', 'Main Address', 'Main Country'
        ]
    
    # Filter to fields that exist in the DataFrame
    available_fields = [field for field in selected_fields if field in df_groups.columns]
    ...
    for group_id in result_df['group_id'].unique():
        ...
        result_df.loc[group_data.index, 'merge_preview_json'] = json.dumps(merge_preview, indent=2)
    
    return result_df


def _generate_group_merge_preview(group_data: pd.DataFrame, fields: List[str]) -> Dict[str, Any]:
    """
    ...
    for field in fields:
        ...
        if len(field_values) > 1:
            ...
            preview["field_comparisons"][field] = {
                ...
                "alternative_values": [str(val) for val in field_values if str(val) != str(primary_record.get(field, ''))],
                "has_conflict": True
            }
        else:
            # Field is consistent
