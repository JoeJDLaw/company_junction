...
def build_raw_exact_key(account_name: str, settings: Dict[str, Any]) -> str:
    ...
    if settings.get("pipeline", {}).get("exact_equals_first_pass", {}).get("key_trim", True):
...
def find_exact_equals_groups(
    ...
    """
    logger.info(f"exact_equals | backend=pandas | records={len(df)} | name_column={name_column}")
    ...
    exact_groups = df_with_keys.groupby("raw_exact_key").agg({
        "account_id": list,
        name_column: list
    }).reset_index()
    
    # Filter groups with size >= min_group_size
    min_group_size = settings.get("pipeline", {}).get("exact_equals_first_pass", {}).get("min_group_size", 2)
    ...
    representative_policy = settings.get("pipeline", {}).get("exact_equals_first_pass", {}).get("representative_policy", "min_account_id")
    ...
    for _, group in exact_groups.iterrows():
        ...
        # Find representative
        rep_data = exact_raw_groups[exact_raw_groups["raw_exact_key"] == raw_exact_key].iloc[0]
    ...
    total_members = sum(len(group["account_id"]) for _, group in exact_groups.iterrows())
...
def write_exact_equals_artifacts(
    ...
    def get_safe_path(base_path: str, suffix: str = "") -> str:
        ...
        if Path(base_path).exists():
            ...
            logger.info(f"exact_equals | existing_file_present | fallback_path={new_path} | reason=no_overwrite_policy")
            return new_path
        return base_path
    ...
    if not exact_raw_groups.empty:
        ...
        logger.info(f"exact_equals | written=exact_raw_groups.parquet | groups={len(exact_raw_groups)} | path={safe_groups_path}")
    ...
    if not raw_exact_map.empty:
        ...
        logger.info(f"exact_equals | written=raw_exact_map.parquet | mappings={len(raw_exact_map)} | path={safe_map_path}")
    ...
    if not candidate_pairs_exact_raw.empty:
        ...
        logger.info(f"exact_equals | written=candidate_pairs_exact_raw.parquet | pairs={len(candidate_pairs_exact_raw)} | path={safe_pairs_path}")
