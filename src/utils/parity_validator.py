...
class ParityValidator:
    ...
    def validate_group_stats_parity(
        ...
        """
        logger.info(f"parity_validator | starting_validation | duckdb_groups={len(duckdb_df)} | pandas_groups={len(pandas_df)}")
        ...
        # Log results
        if is_parity_valid:
            logger.info(f"parity_validator | validation_passed | rows={parity_report['rows_compared']} | mismatches=0")
        else:
            logger.error(f"parity_validator | validation_failed | rows={parity_report['rows_compared']} | mismatches={parity_report['mismatches']}")
    ...
    def _validate_column_parity(
        ...
        if pd.api.types.is_numeric_dtype(duckdb_series) and pd.api.types.is_numeric_dtype(pandas_series):
        ...
        elif pd.api.types.is_string_dtype(duckdb_series) or pd.api.types.is_object_dtype(duckdb_series):
    ...
    def _save_parity_report(self, parity_report: Dict[str, Any], run_id: str) -> None:
        ...
        try:
            ...
            if output_path.exists():
                ...
                logger.info(f"parity_validator | existing_file_present | fallback_path={output_path} | reason=no_overwrite_policy")
            
            # Save report
            with open(output_path, 'w') as f:
                json.dump(parity_report, f, indent=2, default=str)
            
            logger.info(f"parity_validator | report_saved | path={output_path} | mismatches={parity_report['mismatches']}")
