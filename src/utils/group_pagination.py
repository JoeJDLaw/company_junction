...
def get_groups_page_pyarrow(
    ...
    try:
        ...
        if len(present_columns) < len(needed_columns):
            ...
            logger.warning(
                f"Stats columns missing in {parquet_path}: {missing}. Reading all columns.",
            )
    ...
    logger.info(f"PyArrow table sorted | run_id={run_id} elapsed={sort_exec_time:.3f}s")
...
def get_groups_page_duckdb(
    ...
    try:
        ...
        page_sql = (
            ...
            + ", '') AS "
            + PRIMARY_NAME
            + " FROM stats s LEFT JOIN primary_names p USING ("
            + GROUP_ID
            + ") "
...
def get_groups_page_from_stats_duckdb(
    ...
    try:
        ...
        try:
            ...
            stats_columns = [
                col
                for col in [GROUP_ID, GROUP_SIZE, MAX_SCORE, PRIMARY_NAME, DISPOSITION]
                if col in available_columns
            ]

            if not stats_columns:
                raise ValueError(
                    f"No required columns found in group_stats_parquet: {group_stats_path}",
                )
            ...
            _review_ready_path = artifact_paths.get("review_ready_parquet")
            ...
            if False:  # Disabled for now
                ...
                sql = (
                    "SELECT " + ",".join(stats_columns) + " FROM ("
                    "  SELECT " + ",".join(stats_columns) + "  FROM read_parquet(?) "
                    "  WHERE "
                    + where_clause
                    + "  ORDER BY "
                    + order_by
        ...
        elapsed = time.time() - start_time
        logger.error(
            f'Groups page from stats failed | run_id={run_id} error="{e!s}" elapsed={elapsed:.3f}',
        )
        raise
...
def get_total_groups_count_from_stats_duckdb(
    ...
    try:
        ...
        try:
            ...
            total_groups = int(result.fetchone()[0])
