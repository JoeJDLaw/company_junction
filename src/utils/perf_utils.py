...
def parse_name_core_tokens(value: Union[str, list, None]) -> frozenset[str]:
    ...
    if isinstance(value, str):
        ...
        return frozenset(token.strip().strip('"\'') for token in tokens if token.strip())
...
def build_vectorized_masks(
    ...
    if manual_blacklist:
        manual_mask = df['account_name'].str.lower().str.contains(
            '|'.join(manual_blacklist), case=False, na=False
        )
    ...
    # Multiple names
    masks['multi_name_mask'] = df.get('has_multiple_names', pd.Series(False, index=df.index))
...
def apply_vectorized_disposition(
    ...
    dispositions = pd.Series([disposition_map.get(r, "Keep") for r in reasons], index=df.index)
