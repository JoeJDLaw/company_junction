...
def get_settings() -> dict[str, Any]:
    ...
    return {
        ...
        "ui_perf": {
            ...
            "details": {"use_details_parquet": True, "allow_pyarrow_fallback": False},
...
def validate_settings(settings: dict[str, Any] | None = None) -> list[str]:
    ...
    if (
        ...
        warnings.append(
            f"ui.timeout_seconds must be number 1-300, got {timeout_seconds}",
        )

    max_pyarrow_seconds = ui.get("max_pyarrow_groups_seconds", 5)
    if (
        not isinstance(max_pyarrow_seconds, (int, float))
        or max_pyarrow_seconds < 1
        or max_pyarrow_seconds > 60
    ):
        warnings.append(
            f"ui.max_pyarrow_groups_seconds must be number 1-60, got {max_pyarrow_seconds}",
        )
