...
from .filtering import get_order_by, apply_filters_duckdb  # if you want user-facing sort options
from .ui_session import session
from .metrics import record_details_request, record_backend_choice, record_timeout, record_page_size_clamped
...
def get_group_details(
    run_id: str, group_id: str, sort_key: str, page: int, page_size: int, filters: Dict[str, Any]
    ...
    source_path = details_path if details_path and os.path.exists(details_path) else review_ready_path
    ...
    prefer_duckdb = settings.get("ui_perf", {}).get("details", {}).get("duckdb_prefer_over_pyarrow", True)
    allow_pyarrow_fallback = settings.get("ui_perf", {}).get("details", {}).get("allow_pyarrow_fallback", False)
...
def _get_group_details_pyarrow(
    ...
    if wanted_col not in filtered.column_names:
        ...
        logger.warning("Order column '%s' (from order_by=%s) not found; falling back to %s", 
                      wanted_col, order_by, ACCOUNT_NAME)
    ...
    try:
        ...
        if ACCOUNT_NAME in df.columns:
            df = df.sort_values([col, ACCOUNT_NAME], ascending=[ascending, True], na_position="last")
        else:
